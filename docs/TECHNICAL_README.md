...
The interactive map now ships with a checked-in copy of the `us-atlas@3` state topology at `public/data/us-states-topology.json`. The `EnhancedUSAMap` component attempts to load this local asset first so development builds and offline play remain functional; if the fetch fails it surfaces a toast and falls back to the CDN copy, and as a last resort it reverts to simple placeholder polygons.【F:public/data/us-states-topology.json†L1-L1】【F:src/components/game/EnhancedUSAMap.tsx†L102-L183】 Documenting this flow makes it clear that shipping builds do not require live network access for the basic USA geometry.
