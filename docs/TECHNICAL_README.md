...
- **Victory checks:** `winCheck` uses the shared [standard victory evaluator](../src/game/victoryRules.ts): Truth ≥90 for Truth Seekers or ≤10 for Government, then 200 IP, then 10 states. Scenario thresholds override the standard values where supplied. The live screen and simulation share this evaluation.
