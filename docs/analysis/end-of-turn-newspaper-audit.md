...
2. **Article Bank** – `loadArticleBank` first fetches runtime JSON (`data/` or `assets/data/`), then falls back to the statically bundled `paranoid_times_card_articles_ALL.json`. Parsed articles are indexed in a `Map` keyed by card id with a helper for `getArticleById` lookups.【F:src/news/articleBank.ts†L152-L184】
...
## Strengths
- **Robust Fallbacks:** Both the cosmetic pool loader and the article bank supply deterministic fallback text, ensuring the tabloid always renders even when network assets fail or new cards lack dedicated prose.【F:src/lib/newspaperData.ts†L117-L145】【F:src/news/articleBank.ts†L152-L184】
...
3. **Expand Article Metadata Schema** – Extend `paranoid_times_card_articles_ALL.json` (and its loader) with optional fields like `statesMentioned`, `recurringCharacter`, or `followUpHooks` to support richer recap sentences and future cross-linking.【F:src/news/articleBank.ts†L8-L135】
...
1. **Samle datakilder** – Konsolider artikkelbank og kosmetiske pooler i ett delt hook/konfig, og la både avisspreaden og front page-widgeten motta ferdig hydrert dataobjekt fra spilltilstanden.【F:src/components/game/TabloidNewspaperV2.tsx†L347-L384】【F:src/news/articleBank.ts†L152-L184】【F:src/ui/newspaper/FrontPage.tsx†L35-L169】
...
3. **Metadata-utvidelse** – Oppdater `paranoid_times_card_articles_ALL.json` og `cardArticleSchema` med nye felt, og bruk dem til å generere oppfølgingslinjer og teaser-lenker til kampanjearcene.【F:src/news/articleBank.ts†L8-L135】【F:src/components/game/TabloidNewspaperV2.tsx†L827-L959】
...
3. **Schema Update:** Utvid `cardArticleSchema` med nye felt og migrer `paranoid_times_card_articles_ALL.json`; dokumenter hvordan skribenter kan fylle ut metadata for fremtidige kort.【F:src/news/articleBank.ts†L8-L135】
...
- **Fallback-datasett:** Filen pakkes inn i builden og importeres direkte av `loadArticleBank`. Når runtime-JSON ikke er tilgjengelig, sørger denne bunten for at hver kort-ID fortsatt har minst ett artikkelfrø, slik at genererte avisoppslag aldri står uten tekst.【F:src/news/articleBank.ts†L152-L184】
