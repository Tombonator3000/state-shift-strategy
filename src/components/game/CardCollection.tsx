...
const CardCollection = ({ open, onOpenChange }: CardCollectionProps) => {
  const { getDiscoveredCards, getCardStats, getCollectionStats, resetCollection } = useCardCollection();
  ...
  }, [discoveredCards, filterRarity, filterType, searchTerm]);

  const handleCardSelect = (card: GameCard) => {
    setSelectedCard(card);
  };

  const handlePreviewChange = (isOpen: boolean) => {
    if (!isOpen) {
  ...
  const CardItem = ({ card }: { card: GameCard }) => {
    ...
    return (
      <button
        ...
        className="bg-card border border-border rounded-lg p-4 hover:bg-accent/50 transition-colors text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Button variant="outline" size="sm" onClick={resetCollection}>
                Reset Collection
              </Button>
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={selectedCard !== null} onOpenChange={handlePreviewChange}>
        <DialogContent className="max-w-2xl">
          {selectedCard && (
            <div className="space-y-4">
              <DialogHeader>
                <DialogTitle className="flex items-center justify-between gap-4">
                  <span className="text-2xl font-bold">{selectedCard.name}</span>
                  <Badge variant={selectedCard.rarity === 'legendary' ? 'destructive' :
                                selectedCard.rarity === 'rare' ? 'secondary' : 'outline'}>
                    {selectedCard.rarity}
                  </Badge>
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                <Badge variant="outline">Type: {normalizeCardType(selectedCard.type)}</Badge>
                <Badge variant="outline">Cost: {selectedCard.cost} IP</Badge>
                {selectedCard.attack !== undefined && (
                  <Badge variant="outline">Attack: {selectedCard.attack}</Badge>
                )}
                {selectedCard.health !== undefined && (
                  <Badge variant="outline">Health: {selectedCard.health}</Badge>
                )}
              </div>

              <p className="text-base text-foreground whitespace-pre-line">{selectedCard.text}</p>

              {(selectedCard.flavor ?? selectedCard.flavorGov ?? selectedCard.flavorTruth) && (
                <p className="p-4 bg-accent/30 rounded italic text-sm text-muted-foreground">
                  "{selectedCard.flavor ?? selectedCard.flavorGov ?? selectedCard.flavorTruth}"
                </p>
              )}
            </div>
