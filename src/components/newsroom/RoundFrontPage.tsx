...
import type { composeRoundEdition } from '@/systems/news/roundEdition';
export function RoundFrontPage({ edition, truth, ip }: { edition: ReturnType<typeof composeRoundEdition>; truth: number; ip?: number }) {
  return <article className="round-front-page">
    ...
    <aside className="press-results"><b>WHAT ACTUALLY CHANGED</b><p>{edition.outcome}</p>{edition.records.map((record, i) => <div key={`${record.card.id}-${i}`}><span>{record.player === 'human' ? 'YOU' : 'RIVAL'}</span><strong>{record.card.name}</strong><span>{record.targetState ?? 'Nationwide'}</span></div>)}</aside>
