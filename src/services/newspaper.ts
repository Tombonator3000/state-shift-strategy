import type { Article, Card, NewspaperConfig, NewspaperIssue } from "@/types/newspaper";

const G: any = (globalThis as any);
G.__SG_NEWS_QUEUE ??= [] as Article[];
G.__SG_NEWS_CFG ??= null as NewspaperConfig | null;

let _queue: Article[] = G.__SG_NEWS_QUEUE;
let _cfg: NewspaperConfig | null = G.__SG_NEWS_CFG;
...
export async function queueArticleFromCard(card: Card, ctx: { round: number; state: any }) {
  ...
  console.debug("[Newspaper] queued:", { id: card.id, name: card.name, isEvent, title });
}
...
function pickDek(card: Card) { return card.flavorTruth ?? card.flavorGov ?? card.flavor ?? undefined; }
...
function buildBody(card: Card, ctx: { round: number; state: any }): string[] {
  ...
  return [
    `${card.name} was played this round. ${eff ? eff + "." : ""}`,
    tgt ? `Witnesses reported effects targeting ${tgt}.` : undefined,
    `Officials insist results are "within expected anomalies".`,
...
function pickAds(cfg: NewspaperConfig, n: number) {
  const src = cfg.ads ?? []; const sh = [...src].sort(() => Math.random() - 0.5);
  return sh.slice(0, Math.min(n, sh.length));
}
function pickSidebars(cfg: NewspaperConfig, n: number) {
  const src = cfg.sidebars ?? []; const sh = [...src].sort(() => Math.random() - 0.5);
  return sh.slice(0, Math.min(n, sh.length));
}
function shortlineFrom(a: Article): string { return a.title; }
...
function extractPercent(e: any): string {
  if (!e) return ""; const td = (e.truthDelta ?? 0);
  return typeof td === "number" ? String(Math.abs(Math.round(td))) : "";
}
