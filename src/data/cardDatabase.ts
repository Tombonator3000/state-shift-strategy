...
// Try to import from core collector, fallback to minimal set
let CORE_CARDS: GameCard[] = [];

try {
  // Import core collector (this will gather all batch files)
  const coreModule = await import('./core');
  CORE_CARDS = coreModule.CARD_DATABASE_CORE || [];
  console.log(`✅ [RUNTIME RECOVERY] Loaded ${CORE_CARDS.length} cards from core collector`);
} catch (error) {
...
// Normalize all core cards and export
export const CARD_DATABASE: GameCard[] = CORE_CARDS.map(normalize);
...
if (import.meta.env.DEV) {
  ...
  if (CARD_DATABASE.length < 100) {
    console.warn('⚠️ Core database seems incomplete. Expected ~400 cards, got', CARD_DATABASE.length);
