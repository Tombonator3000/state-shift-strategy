...
describe('assignStateBonuses deterministic selection', () => {
  const mockStates = [
  ...
  test('assignments stay stable for identical seed/round', () => {
    ...
    expect(resultA.debug && (resultA.debug as { seed?: number }).seed).toBe(
      resultB.debug && (resultB.debug as { seed?: number }).seed,
    );
  ...
  test('different rounds produce different roll signatures', () => {
    ...
    const seedFive = roundFive.debug && (roundFive.debug as { seed?: number }).seed;
    const seedSix = roundSix.debug && (roundSix.debug as { seed?: number }).seed;

    expect(seedFive).not.toBe(seedSix);
  });
  ...
  test('newspaper events carry trigger metadata scaled by event chance', () => {
    const eventChance = 1;
    const result = assignStateBonuses({
      states: mockStates,
      baseSeed: 1,
    ...
    for (const event of result.newspaperEvents) {
      ...
      expect(event.triggerChance).toBeCloseTo(eventChance);
      const expectedConditional = Math.min(1, eventChance / normalizedWeight);
      expect(event.conditionalChance).toBeCloseTo(expectedConditional);
    }
  });

  test('truth and IP totals split between human- and AI-controlled states', () => {
    const states = [
    ...
    expect(result.logs.some(entry => entry.startsWith('Truth sway'))).toBe(true);
  });
  ...
  test('government players invert truth adjustments for their controlled states', () => {
    ...
    expect(
      governmentFactionResult.playerTruthDelta + truthFactionResult.playerTruthDelta,
    ).toBe(0);
    expect(
      governmentFactionResult.aiTruthDelta + truthFactionResult.aiTruthDelta,
    ).toBe(0);
