...
import { composeRoundEdition } from './roundEdition';
...
export function dispatchForPlay(record: CardPlayRecord): PressDispatch {
  ...
  return { id: `${record.round}:${record.turn}:${record.timestamp}:${record.card.id}`, kind: record.faction === 'government' ? 'redacted' : 'breaking', title: story.headline, body: story.body,
    outcome: `Truth ${signed(record.truthDelta)}% · Your IP ${signed(record.ipDelta)} · Rival IP ${signed(record.aiIpDelta)}${record.capturedStates.length ? ` · Captured: ${record.capturedStates.join(', ')}` : record.targetState ? ` · Target: ${record.targetState}` : ''}`,
